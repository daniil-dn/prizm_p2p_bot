...
async def continue_order(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
...
async def stop_order(callback: CallbackQuery, button: Button, dialog_manager: DialogManager):
    session = dialog_manager.middleware_data['session']
    order = await crud_order_request.get_by_id(session, id=int(dialog_manager.dialog_data['order_id']))
