...
from ..ui.menu import get_menu_personal_area_kb, get_meny_buy_sell_mode_kb, get_meny_buy_mode_kb, get_meny_sell_mode_kb
...
@router.callback_query(F.data == ("menu-mode-buy-sell"))
async def menu_buy_sell(callback: CallbackQuery, bot: Bot, state: FSMContext, user_db: User,
                       dialog_manager: DialogManager, session: AsyncSession) -> None:
    await dialog_manager.reset_stack(remove_keyboard=True)
    await state.clear()
    await bot.send_message(
        user_db.id, get_start_text(user_db.balance, user_db.referral_balance, user_db.order_count, user_db.cancel_order_count),
        reply_markup=get_meny_buy_sell_mode_kb()
    )

@router.callback_query(F.data == ("menu-mode-buy"))
async def menu_buy_sell(callback: CallbackQuery, bot: Bot, state: FSMContext, user_db: User,
                       dialog_manager: DialogManager, session: AsyncSession) -> None:
    await dialog_manager.reset_stack(remove_keyboard=True)
    await state.clear()
    await bot.send_message(
        user_db.id, get_start_text(user_db.balance, user_db.referral_balance, user_db.order_count, user_db.cancel_order_count),
        reply_markup=get_meny_buy_mode_kb()
    )

@router.callback_query(F.data == ("menu-mode-sell"))
async def menu_buy_sell(callback: CallbackQuery, bot: Bot, state: FSMContext, user_db: User,
                       dialog_manager: DialogManager, session: AsyncSession) -> None:
    await dialog_manager.reset_stack(remove_keyboard=True)
    await state.clear()
    await bot.send_message(
        user_db.id, get_start_text(user_db.balance, user_db.referral_balance, user_db.order_count, user_db.cancel_order_count),
        reply_markup=get_meny_sell_mode_kb()
    )
