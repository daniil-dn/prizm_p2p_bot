...
from app.bot.ui import recieved_card_transfer
...
async def accept_card_transfer_recieved_cb(cb: CallbackQuery, bot: Bot, state: FSMContext, user_db: User,
    ...
    async with session:
        ...
        prizm_value = order.from_value if order.mode == 'sell' else order.to_value
    ...
    await cb.message.reply("Вы подтвердили оплату. Сделка завершается ")
    ...
    async with session:
        ...
        await crud_user.update(session, db_obj=buyer,
