...
from app.bot.ui.order_seller_accept import order_request_sended_pzm_transfer_button
...
async def create_order_and_wallet(dialog_manager: DialogManager, user_id: int, sell_card_info: str):
    ...
    if dialog_manager.start_data['mode'] == 'sell':
        ...
        if dialog_manager.dialog_data['card_method'] == "sbp":
            wallet_type = OrderRequest.WALLET_SBP
        else:
            wallet_type = OrderRequest.WALLET_CARD
        order_request_status = OrderRequest.WAIT_PRIZM
    else:
        from_currency = "RUB"
        to_currency = "PRIZM"
        wallet_currency = 'PRIZM'
        wallet_type = OrderRequest.WALLET_PZM
...
async def send_notification(dialog_manager: DialogManager, admin_settings: Settings, order_request: OrderRequest,
    ...
    if dialog_manager.start_data['mode'] == 'sell':
        ...
        text = (
            f"Переведите {value_with_commission:.2f} PZM c коммиссией сервиса {admin_settings.commission_percent * 100}%"
        ...
        await message.bot.send_message(message.from_user.id, text=text, parse_mode='html',
                                       reply_markup=order_request_sended_pzm_transfer_button(message.from_user.id,
                                                                                             order_request.id))
